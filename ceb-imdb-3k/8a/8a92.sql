...
AND (it1.id IN ('3'))
AND (mi1.info in ('Action','Adventure','Animation','Crime','Documentary','Family','Romance','Thriller'))
AND (kt.kind in ('movie'))
AND (rt.role in ('cinematographer','composer','costume designer','director','editor','miscellaneous crew','producer','writer'))
AND (n.gender IS NULL)
AND (n.surname_pcode in ('B624','B65','J52','J525','R2') OR n.surname_pcode IS NULL)
